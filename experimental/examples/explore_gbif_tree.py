#!/usr/bin/env python3
"""Interactive text-based GBIF taxonomy tree explorer.
...
from taxonomica.gbif_tree import RANK_PRIORITY, GBIFTaxonomyTree, TaxonomyNode
...
class TreeExplorer:
    ...
    def __init__(self, tree: GBIFTaxonomyTree, start_node: TaxonomyNode | None = None) -> None:
        self.tree = tree
        self.current_node = start_node or tree.root
        self.page = 0
        self.page_size = 26
        self.history: list[TaxonomyNode] = []
        self.sort_mode = self.SORT_RANK
        self.filter_complete_paths = True
    ...
    def get_breadcrumb(self) -> str:
        ...
        if len(path) <= 1:
            return "Life"
        parts = [node.name for node in reversed(path[:-1])]
        return " → ".join(parts)
    ...
    def display(self) -> None:
        ...
        if ancestors:
            print("  Ancestors:")
            for i, anc in enumerate(reversed(ancestors[-5:])):
                indent = "    " * (i + 1)
        ...
        if self.current_node != self.tree.root:
            ...
            if self.current_node.vernacular_names:
                print(f"  │  Common: {self.current_node.vernacular_names[0]}")
...
def main() -> None:
    backbone_path = Path(__file__).resolve().parents[2] / "assets" / "raw" / "gbif-backbone"
